...
TOPICS: dict[str, tuple[str, ...]] = {
    ...
    "going_concern": ("계속기업",),
    # 아래 셋은 이상징후 후속조사 전용이다. 판정 25항목은 쓰지 않는다.
    #
    # find_subsection 은 첫 일치를 돌려주고 각 항목은 자기 topic 만 묻기 때문에
    # 여기에 늘려도 기존 조회 결과가 바뀌지 않는다(순수 가산).
    #
    # '유동성' 과 '만기' 를 함께 두는 이유: 센트비 주석 8 은 '금융부채의 유동성위험
    # 관리방법 및 종류별 만기 분석' 이라 어느 쪽으로도 걸린다. 제목 앞 30자만 보므로
    # 본문에서 지나가듯 쓴 '만기' 에는 걸리지 않는다.
    "liquidity": ("유동성", "만기"),
    "receivable": ("매출채권", "미수금"),
    "deficit": ("결손금",),
}
